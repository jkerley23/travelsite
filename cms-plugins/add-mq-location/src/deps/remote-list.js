(function($, document, undefined) {
  'use strict';
  // https://github.com/aFarkas/remote-list commit a24cf64 with modifications.
  // Search for "MODIFIED" for changes.
  ...
  RemoteList.prototype = {
    ...
    _bindEvents: function(inst) {
      ...
      var detectListselect = (function() {
        // MODIFIED: Removed curValue -> lastValue comparison / behavior. This is handled in above
        // implementation and was causing bugs.
  ...
  $.fn.remoteList = function(opts,
    ...
    this.each(function() {
      ...
      if (
        inst && $.isPlainObject(inst)) {
        markupOpts = inst;
      }
      if (
        inst && typeof fn == 'string' && fn in inst) {
        instRet = inst[fn] && inst[fn
        ].apply ? inst[fn].apply(inst, args || []) : inst[fn];
        if (
          instRet !== undefined) {
          ret = instRet;
          return false;
        ...
        $(this).data(
          'remoteList', new RemoteList(this, $.extend({}, opts,
            markupOpts)));
      }
    });
...
})(window.webshims && webshims.$ || window.jQuery || window.$, window.document);
